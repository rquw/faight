...
(function (root) {
  ...
  // angles go over the wire as one byte (1.4 deg steps) - at 20 players that saves ~30 % of a snapshot
  const A2B = 128 / Math.PI, B2A = Math.PI / 128;
  const packA = (a) => { const v = Math.round(wrap(a) * A2B); return v > 127 ? -128 : v < -128 ? -128 : v; };
  ...
  function encode(tm, players, props, items, projs) {
    const size = 1 + 4 + 2 + players.length * 22 + 2 + props.length * 7 + 2 + items.length * 9 + 2 + projs.length * 8;
    ...
    for (const p of players) {
      ...
      for (let i = 0; i < 11; i++) { v.setInt8(o, packA(p.angles[i])); o += 1; }
    }
    v.setUint16(o, props.length); o += 2;
    for (const [id, x, y, a] of props) { v.setUint16(o, id); v.setInt16(o + 2, clamp16(x * 100)); v.setInt16(o + 4, clamp16(y * 100)); v.setInt8(o + 6, packA(a)); o += 7; }
    v.setUint16(o, items.length); o += 2;
    for (const [id, t, x, y, a, st] of items) { v.setUint16(o, id); v.setUint8(o + 2, t); v.setInt16(o + 3, clamp16(x * 100)); v.setInt16(o + 5, clamp16(y * 100)); v.setInt8(o + 7, packA(a)); v.setUint8(o + 8, st); o += 9; }
    v.setUint16(o, projs.length); o += 2;
    for (const [id, t, x, y, a] of projs) { v.setUint16(o, id); v.setUint8(o + 2, t); v.setInt16(o + 3, clamp16(x * 100)); v.setInt16(o + 5, clamp16(y * 100)); v.setInt8(o + 7, packA(a)); o += 8; }
    return buf;
  }
  ...
  function decode(buf) {
    ...
    for (let k = 0; k < n; k++) {
      ...
      for (let i = 0; i < 11; i++) { A[i] = v.getInt8(o) * B2A; o += 1; }
    ...
    n = v.getUint16(o); o += 2;
    for (let k = 0; k < n; k++) { O.push([v.getUint16(o), v.getInt16(o + 2), v.getInt16(o + 4), v.getInt8(o + 6) * B2A * 100]); o += 7; }
    n = v.getUint16(o); o += 2;
    for (let k = 0; k < n; k++) { I.push([v.getUint16(o), v.getUint8(o + 2), v.getInt16(o + 3), v.getInt16(o + 5), v.getInt8(o + 7) * B2A * 100, v.getUint8(o + 8)]); o += 9; }
    n = v.getUint16(o); o += 2;
    for (let k = 0; k < n; k++) { R.push([v.getUint16(o), v.getUint8(o + 2), v.getInt16(o + 3), v.getInt16(o + 5), v.getInt8(o + 7) * B2A * 100]); o += 8; }
    return { tm, P, O, I, R };
  }
